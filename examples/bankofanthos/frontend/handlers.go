...
func (fe *Server) homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	balance, err := fe.balanceReader.GetBalance(r.Context(), accountID)
	if err != nil {
		logger.Error("Couldn't fetch balance", err)
	}
	txnHistory, err := fe.transactionHistory.GetTransactions(r.Context(), accountID)
	if err != nil {
		logger.Error("Couldn't fetch transaction history", err)
	}
	contacts, err := fe.contacts.GetContacts(r.Context(), username)
	if err != nil {
...
func (fe *Server) paymentHandler(w http.ResponseWriter, r *http.Request) {
	...
	if recipient == "add" {
		...
		if label != "" {
			...
			err := fe.contacts.AddContact(r.Context(), authenticatedAccountID, contact)
			if err != nil {
	...
	err = fe.ledgerWriter.AddTransaction(r.Context(), r.FormValue("uuid"), authenticatedAccountID, txn)
...
func (fe *Server) loginPostHelper(w http.ResponseWriter, r *http.Request) error {
	...
	token, err := fe.userService.Login(r.Context(), loginReq)
	if err != nil {
		return err
	}
...
func (fe *Server) signupPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := fe.userService.CreateUser(r.Context(), creq)
	if err != nil {
